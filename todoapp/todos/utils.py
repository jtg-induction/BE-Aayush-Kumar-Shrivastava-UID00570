...
from projects.serializers import (
    ProjectSerializer,
    ProjectWithMemberNameSerializer,
    ProjectReportSerializer
)
from todos.serializers import (
    TodoSerializer,
    TodosWithinDateRangeSerializer
)
from users.serializers import (
    UserSerializer,
    UserTodoStatsSerializer,
    PendingTodosSerializer,
...
def fetch_all_todo_list_with_user_details():
    ...
    """
    todos_data = Todo.objects.select_related('user')

    serializer = TodoSerializer(todos_data, many=True)
    return json.loads(json.dumps(serializer.data))
...
def fetch_projects_details():
    ...
    """
    projects_data = Project.objects.annotate(
...
def fetch_five_users_with_max_pending_todos():
    ...
    """
    users_data = CustomUser.objects.annotate(
        pending_count=Count('todo', filter=Q(todo__done=False))
    ).order_by('-pending_count')[:5]

    serializer = PendingTodosSerializer(users_data, many=True)
    return json.loads(json.dumps(serializer.data))
...
def fetch_completed_todos_with_in_date_range(start, end):
    ...
    serializer = TodosWithinDateRangeSerializer(
        todos, many=True)
    return json.loads(json.dumps(serializer.data))
...
def fetch_project_with_member_name_start_or_end_with_a():
    ...
    serializer = ProjectWithMemberNameSerializer(projects, many=True)
    return json.loads(json.dumps(serializer.data))
...
def fetch_project_wise_report():
    ...
    projects = Project.objects.prefetch_related(members_prefetch)
...
def fetch_user_wise_project_status():
    ...
    serializer = UserWiseProjectStatusSerializer(users_data, many=True)
    return json.loads(json.dumps(serializer.data))
